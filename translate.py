...
tf.app.flags.DEFINE_boolean("serve", False,
                            "Run bot")
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                for bucket_id in xrange(len(_buckets)):
                    if len(dev_set[bucket_id]) == 0:
                        logging.warning("  eval: empty bucket %d" % (bucket_id))
                        continue
                    encoder_inputs, decoder_inputs, target_weights = model.get_batch(
                        dev_set, bucket_id)
...
def telegram_error_callback(bot, update, error):
...
def serve():
    with tf.Session() as sess:
        ...
        updater = Updater("378949807:AAF17KNFdQoZh8ktysAshGShnza-BW2uPeY")
        dp = updater.dispatcher
        dp.add_error_handler(telegram_error_callback)

        def serve_worker(bot, u):
            sentence = u.message.text.lower()
            # Get token-ids for the input sentence.
            token_ids = data_utils.sentence_to_token_ids(sentence, en_vocab)
            # Which bucket does it belong to?
            bucket_id = len(_buckets) - 1
            ...
            if data_utils.EOS_ID in outputs:
                outputs = outputs[:outputs.index(data_utils.EOS_ID)]
            # Print out French sentence corresponding to outputs.
            u.message.reply_text(" ".join([rev_fr_vocab[output] for output in outputs]))

        dp.add_handler(MessageHandler(Filters.text, serve_worker))
